 ...
 *
 * <p>These verify that probe chains stay intact when entries are removed
 * from the middle of chains, and that the map remains consistent after
 * repeated removals and re-insertions.</p>
 */
class IntToIntMapShiftTest {
    ...
    void entrySetIteratorReflectsShiftedChains() {
        ...
        assertTrue(
            entries
                .stream()
                .anyMatch(e -> e.getKey() == 0x20 && e.getValue() == 100)
        );
        assertTrue(
            entries
                .stream()
                .anyMatch(e -> e.getKey() == 0x60 && e.getValue() == 300)
        );
        assertTrue(
            entries
                .stream()
                .anyMatch(e -> e.getKey() == 0x80 && e.getValue() == 400)
        );
    }
    ...
    void stressRemoveAndReAddAfterShift() {
        ...
        for (int round = 0; round < 5; round++) {
            ...
            for (int i = 0; i < 50; i++) {
                ...
                assertTrue(
                    map.containsKey(key),
                    "round " + round + " should contain " + key
                );
            }
