...
/**
 * A set backed by an {@code int[]} where each array element stores 32
 * boolean values as individual bits.  A bit set to {@code 1} means the
 * corresponding index is present in the set.
 *
 * <p>Implements {@link Set<Integer>} so it can be used wherever a standard
 * set is expected.  Elements are non-negative integers.</p>
 */
public final class IntBitSet implements Set<Integer> {
    ...
    /**
     * Constructs a set that can hold {@code capacity} boolean values
     * (indexed from 0 to {@code capacity - 1}).
     *
